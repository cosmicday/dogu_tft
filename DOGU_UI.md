...
- **★ 터치 기기에서는 그 높이를 JS 가 px 로 못 박는다 (2026-09-18, pixlol 폰 크롬).** 폰 크롬은 키보드가 열릴 때
  뷰포트 자체를 줄였다 되돌려서 `100lvh` 도 따라 바뀌고, `cover` 가 그림을 다시 확대·축소해 **검색창에 포커스했다 풀면 배경이 움찔**한다
  (사파리는 키보드가 뷰포트를 안 줄여 무증상). `dogu-header.js` 끝의 `freezeBgHeight` 가 `(hover: none) and (pointer: coarse)` 일 때만
  큰 뷰포트 높이를 재서 `--dogu-bg-h` 와 `body.dogu-vh-fixed` 를 붙인다 — 같은 폭에서 **줄어드는** 변화(키보드)는 무시, **커지는**
  변화(주소창 숨김)는 채택, 폭이 바뀌면(회전) 다시 잰다. 데스크톱은 안 걸린다(창 높이를 줄이면 그림이 넘치므로). 사이트가 할 일은 없다.
