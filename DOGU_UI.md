...
| `--dogu-toast-bg` / `-line` / `-fg` / `-sub` / `-ico` / `-ico-bg` | 카드색 · `--dogu-line-2` · 글자색 · `--dogu-text-3` · 액센트 | 복사 안내 토스트(6절)의 바탕·테두리·제목·부제·아이콘 색. **안 주면 카드색 그대로** (pixlol 만 보라 계열, 2026-09-18) |
| `--dogu-search-dd-bg` / `-head` / `-line` / `-border` | 카드색 · `--dogu-card-2` · `--dogu-line` · `--dogu-line-2` | 검색 드롭다운(즐겨찾기·최근검색)의 바탕·머리·줄 사이 선·테두리. 위와 같은 규칙 (2026-09-18) |
...
- **★★ 터치 기기에서는 그림 층이 `fixed` 가 아니라 `absolute`(문서에 붙음)다 (2026-09-18, pixlol 폰 크롬 — 사용자 결정).**
  폰 크롬은 주소창이 **위**에 있어서 맨 위에서 조금 내리는 동안 주소창이 접히며 웹 화면의 위 가장자리가 밀려 올라가고, `fixed; top: 0` 층은
  거기 붙어 같이 올라간다 (조금 내릴 때까지만 배경이 따라오고 그 뒤 고정). 키보드가 뜨고 내릴 때도 주소창을 접었다 펴서 **배경이 움찔**한다.
  사파리는 주소창이 아래라 무증상. **높이만 px 로 못 박는 첫 시도로는 안 잡혔다** (크기가 아니라 위치가 따라가는 것이라).
  그래서 `dogu-header.js` 끝의 `freezeBgHeight` 가 `(hover: none) and (pointer: coarse)` 일 때 `body.dogu-vh-fixed` 를 붙이고, CSS 가 그 층을
  `position: absolute; height: var(--dogu-bg-h)` 로 바꾼다 — 폰에서는 보통 히어로처럼 **그림이 페이지와 같이 스크롤**된다.
  오버레이(`::after`)는 색뿐이라 `fixed` 그대로라 아래로 내려간 곳은 전처럼 진하다. 높이 px 는 큰 뷰포트를 재서 준다(같은 폭에서 줄어드는
  변화는 무시 · 커지는 변화는 채택 · 폭이 바뀌면 다시 잼) — `lvh` 로 두면 키보드가 뷰포트를 줄이는 브라우저에서 `cover` 가 다시 확대된다.
  데스크톱은 클래스가 안 붙어 `fixed + 100lvh` 그대로. 사이트가 할 일은 없다.
  안드로이드 크롬은 덤으로 viewport 메타에 `interactive-widget=resizes-visual` 을 붙이면 키보드가 레이아웃 뷰포트를 아예 안 건드린다 (사이트 `index.html` 몫, pixlol 적용)
...
- **스위처 ↔ 햄버거 상호 배타 (2026-08-25)**: 둘 다 클릭에 `stopPropagation` 을 걸어 서로의 바깥 클릭 리스너가 안 돌므로, 한쪽을 열 때 다른 쪽을 명시적으로 닫는다 (`bindSwitcher(root, onOpen)` 가 닫기 함수를 돌려주고, 햄버거 클릭이 그걸 부른다). 게임 목록을 펼친 채 메뉴를 열면 둘이 겹쳐 보이던 문제.
- **★ 폰 메뉴 패널 배경 `--dogu-gnb-panel` (2026-08-27)**: 펼친 세로 메뉴는 2단 바와 같은 반투명(`--dogu-gnb-nav`)이라 **뒤 페이지 글자가 메뉴 글자와 겹쳐 읽힌다** (항목이 늘수록 심하다). 불투명 헤더를 쓰는 사이트는 `--dogu-gnb-panel` 을 덮어쓴다 (pixlol `#16112a`). 안 주면 예전 그대로.
- **★ 하위 메뉴 (2026-08-27)**: `nav[]` 항목에 `sub: [{ key, label, href }]` 를 주면 그 탭이 드롭다운이 된다 (pixlol 의 「도감」 → 챔피언·아이템·룬·소환사 주문).
  **부모에 `href` 를 안 주면 링크가 아니라 `<span>` 이라 클릭해도 아무 데도 안 가고 하위만 연다**(주면 예전처럼 링크). 여는 건 CSS 의 `:hover`/`:focus-within` 이라 여는 JS 가 없고,
  **`setActiveNav(key)` 는 하위가 켜지면 부모 탭도 같이 켠다.** 폰(≤768px)에서는 접지 않고 세로 메뉴 안에 **들여써서 다 펼친다** (캐럿 `▾` 는 숨긴다).
  **★ `.dogu-nav` 가 `overflow-x: auto` 라 드롭다운이 그 안에서 잘린다** — 하위 메뉴가 있을 때만 `.dogu-nav.has-sub { overflow: visible }` 로 푼다 (안 쓰는 사이트는 네비 가로 스크롤이 그대로다).
  `sub` 를 안 넘기면 **마크업이 예전과 한 글자도 다르지 않다**
  - **★ 하위 항목을 누른 뒤 드롭다운이 안 닫히던 것 (2026-08-28 수정)**: 열림이 CSS `:hover`/`:focus-within` 뿐이라 SPA 에서 페이지가 바뀌어도
    누른 `<a>` 의 포커스와 마우스가 그대로면 메뉴가 남았다. 클릭 시 JS 가 `blur()` + 그룹에 `.dogu-sub-hold` 를 붙이고
    `mouseleave` 에 뗀다 (CSS 선택자에 `:not(.dogu-sub-hold)`). `sub` 없는 사이트는 그룹이 없어 코드가 안 돈다
- **★ `html { scrollbar-gutter: stable }` (2026-08-28)**: 내용이 창보다 짧은 페이지에서 세로 스크롤바가 사라지면 문서가 15px 넓어져
  가운데 정렬된 헤더·제목이 긴 페이지와 7~8px 어긋났다. 스크롤바 자리를 항상 비워 둬서 막는다. 짧은 페이지 오른쪽에 15px 빈 띠가 남는 게 대가고, 폰(오버레이 스크롤바)은 영향 없다
- **모바일 이중 헤더 정리 (2026-08-24)**: 768px 이하에서 2단 네비(`.dogu-gnb-main`)를 접고, 1단 오른쪽에 햄버거(`.dogu-menu-btn`, 세 줄)가 생긴다. 누르면 헤더에 `dogu-menu-open` 클래스가 붙고 2단이 1단 아래 **세로 메뉴 패널**(absolute)로 펼쳐진다. 바깥 클릭·메뉴 항목 클릭이 닫는다 (사이트 라우터가 preventDefault 만 하고 전파를 살려 두는 규약이라 document 리스너가 받는다). **마크업·`data-nav`·라우터 연결은 데스크톱과 같은 요소**라 `setActiveNav` 가 그대로 동작한다. `.dogu-nav-home`(⌂)은 모바일에서 숨긴다 — 홈은 로고가 맡는다
  - **★ 패널은 `max-height: calc(100vh - 100%)` + `overflow-y: auto` 다 (2026-08-27).** 항목이 많으면 패널이 화면보다 길어지는데, 헤더가 sticky 라 **페이지를 스크롤해도 패널이 같이 따라와서 넘친 항목에 영원히 닿지 못한다** (maple 이 네비에 하위 메뉴를 달아 20줄 929px 이 되면서 마지막 항목을 못 눌렀다). 패널 자신이 스크롤을 갖게 해서 막는다 — 이 두 줄을 지우면 항목 많은 사이트에서 조용히 재발한다
  - **★ 펼친 패널 배경은 불투명이어야 한다.** 기본값은 2단 바와 같은 반투명(`--dogu-gnb-nav`)이라 **뒤 페이지 글자가 메뉴 글자와 겹쳐 읽힌다.** 사이트가 `--dogu-gnb-panel` 로 불투명 색을 준다 (pixlol `#16112a`, maple 은 자기 `--popup`)
...
사이트가 링크를 **뒤에** 더 붙일 수 있다 — `extraLinks: [{ id, text, href, onClick }]` (2026-08-26, pixlol 「PC 버전」 스위치). 공통 셋 뒤에만 붙고 문구·동작은 사이트 몫이다. 없으면 아무것도 안 그린다.

**「버그제보 및 피드백」은 `mailto` 가 아니라 클립보드 복사다.** 누르면 `contact` 이메일을 클립보드에 복사하고 "이메일 주소(…)가 클립보드에 복사되었습니다." 안내를 띄운다 (실패 시 "복사에 실패했습니다. 직접 복사해 주세요: …"). 안내는 공통 파일의 토스트(`.dogu-toast` — 2026-09-18 부터 ✓/✕ 원형 아이콘 + 제목 + 부제 카드, **1.1초** 뒤 사라짐)가 띄우고, 사이트에 자기 토스트가 있으면 `notify` 로 넘겨 그걸 쓴다. 전 사이트 같은 동작이어야 한다.
★ `notify` 에는 문자열이 아니라 `{ kind: 'ok'|'fail', title, body }` 가 온다 (2026-09-18 부터). `toString` 이 「제목 — 부제」를 돌려주므로 `textContent` 에 그대로 넣는 사이트는 손댈 게 없다. 두 줄로 그리고 싶으면 `title`·`body` 를 따로 쓰면 된다.
★ 드롭다운(즐겨찾기·최근검색)은 검색창·드롭다운 **밖을 누르면 닫힌다** (`document` `pointerdown`/`touchstart` capture). 사이트가 따로 할 일은 없다.
...
## 13. 글자 크기 계단 (2026-09-05 확정 — 근거·실측은 `dogu_template/audit/font-scale.md`)

5개 사이트와 공통 파일이 같은 계단을 쓴다. **`font-size` 에 아래 값만 쓴다.** 새 값이 필요하면 이 표를 먼저 고친다.

| px | 역할 |
|---|---|
| (9 · 10) | **글리프만** — 캐럿 ▾ · 정렬 ▲▼ · 별 ★ · 접기 › . 글자에는 금지 |
| **11** | **미세 라벨만** — 그래프 축 눈금 · SVG 텍스트 · 초상(20~24px) 안 숫자 · 원형 배지. 읽으라고 둔 문장에는 금지 |
| **12** | 보조 — 캡션 · 메타 · 날짜 · 배지 · 칩 · 표 머리 · 각주. **실글자 최소선** |
| **13** | 본문 — 공통 파일 뿌리 · 드롭다운 항목 · 밀도 높은 표 셀 |
| **14** | 본문 강조 · 표 셀 · 컨트롤 · 네비 항목 |
| **15** | 소제목 — 섹션·카드 제목 · 폰 햄버거 항목 |
| **17** | 제목 — 카드 큰 제목 · 강조 수치 · 검색 버튼 |
| **20** | 페이지 소제목 · KPI |
| **24** | 페이지 제목 · 캐릭터/소환사 이름 |
| **28** | 큰 표시 — 랭킹 제목 · 큰 KPI |
| 36 · 52 … | 표시용(히어로 마크 · 아바타) — 계단 밖, 사이트 자유 |

규칙:

- **0.5 단위(11.5 · 12.5 · 13.5) 금지.** 위계로 안 읽히고 "여긴 몇이지" 만 남긴다.
- **폰 전용 단은 없다.** 폰도 같은 계단. 폰에서 값을 바꾸는 규칙은 세 가지뿐 — ① 입력창 `16px` (iOS 사파리 포커스 확대 방지, 계단에 16 이 없어도 이 자리는 16) ② 표시용 축소(브랜드 21→18 · 히어로 52→36 류) ③ 표→카드 전환에 묶인 라벨(loa 620px). **"폰이라서 더 작게" 는 금지.**
...
- **밀도 표의 좁은 셀 패딩** (2026-09-12 등록) — pixlol 밀도 표(19칸 등)는 14-4 의 `th 10px 12px` / `td 9px 12px` 를 주면 **폰에서 가로로 넘친다.** 한 화면에 열을 다 넣으려고 좁힌 자리는 규격 밖. 폰에서 열을 줄이는 쪽은 정보를 없애는 대가라 택하지 않았다.
- **표 안의 컴팩트 셀렉트·입력** (2026-09-12 등록) — maple 계산기 표(해방·심볼 등)의 셀 안 컨트롤은 14-3 의 34px 밖. 34 로 올리면 **행마다 12px 씩** 커져 수십 행이 한 화면에서 밀려난다. 표 밖의 독립 컨트롤은 그대로 34.
- **숫자+단위가 섞인 칸의 열은 14-4 「숫자 열 오른쪽 정렬」 대상이 아니다** (2026-09-12 등록) — maple 계산기 표의 `8개 (40억 메소)` · `3.33회` 처럼 값과 단위가 한 칸에 있거나 `<select>`/`<input>` 이 든 열은 오른쪽으로 밀면 꼬리 길이가 제각각이라 **더 들쭉날쭉해진다.** 14-4 의 규칙은 "값끼리 자릿수를 비교하는 열"이 대상이다. 이런 열은 왼쪽 정렬 + `tabular-nums` 까지.
- **maple `.member` 는 칩이 아니라 「누를 수 있는 이름 타일」** (2026-09-12 등록) — 누를 수 있는 건 맞지만 "이름 / 레벨·직업" 을 **두 줄로 푸는 변형**이 있어 14-3 칩의 `height: 28px` 고정이 안 들어간다. 칩 규격으로 통일하지 않는다.
- **초상·아바타의 모서리 둥글기는 사이트 자유** (2026-09-12 등록) — 14-4 의 `--dogu-r-card` 는 카드·패널·표 래퍼 규칙이지 초상 규칙이 아니다. pixlol `.profile-icon` 85×85 의 반경 15 를 10 으로 내리면 아바타가 눈에 띄게 각져진다. 알아볼 수 있는 모양이 규격보다 앞선다.
- **게임 등급색은 팔레트 토큰이 아니다 — 대비 미달이면 「색상은 두고 밝기만」** (2026-09-12 등록) — loa 아이템 등급색 8개는 인게임 색이라 사용자가 색으로 등급을 읽는다. 미달이던 둘만 흰색을 조금 섞어 올렸다(`.epic #a44fd1` 4.01 → `#ad61d6` 4.74 · `.rare #3f7ed1` 4.44 → `#4683d3` 4.73). 나머지 여섯은 5.95~11.23 으로 통과라 그대로. **팔레트를 다시 잡는 것이 아니라 미달분만 최소로 올린다.**
- **비활성(disabled·off) 흐림은 대비 기준 대상이 아니다** (2026-09-12 등록) — WCAG 가 비활성 컴포넌트를 명시적으로 제외하고, 여기서 흐림은 꾸밈이 아니라 **"못 쓴다 / 안 세었다" 는 정보**다. 선명하게 올리면 활성과 구분이 흐려져 뜻이 손상된다 (loa `button:disabled` .45 · `.ark-opt.off` .42 · `.wg-char.off` .5 · `.tool-card.off` .6). 판단선: **값을 읽어야 하는 것만 올린다.**
