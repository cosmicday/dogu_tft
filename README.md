...
- **랭킹** — 챌린저~마스터 상위 1000명, 10분 주기 갱신. 닉네임은 백그라운드
  ...
  누적되므로 재시작해도 유지).
- **통계 페이지** — 유닛/시너지/아이템별 픽률·평균 등수·순방률·1위율과
  유닛별 추천 아이템 3종. 상위 300 랭커의 랭크 매치를 5분마다 조금씩 수집해
  (개발 키 한도 내) 30분마다 Mongo 집계로 갱신한다. 세트 번호는 쌓인 매치의
  다수결로 판별하므로 세트가 바뀌면 자동으로 따라간다.
- **소환사 페이지 부가 기능** — 최근 20게임 등수 추이 그래프(SVG),
  큐 필터(전체/랭크/일반/더블업/초고속), 전적 갱신 버튼(`?refresh=1`,
  IP당 30초 쿨다운).
